            ...
            if (Auth::validToken($this->db,$this->token,$this->username)){
                ...
                if ($role == 1){
                    try {
                        ...
                        $sql = file_get_contents(dirname(__FILE__).'/customer.sql');
        ...
        public function uninstall(){
            if (Auth::validToken($this->db,$this->token,$this->username)){
                ...
                if ($role == 1){
                    try {
                        $this->db->beginTransaction();
                        $sql = "DROP TABLE IF EXISTS customer_company;DROP TABLE IF EXISTS customer_member;DROP TABLE IF EXISTS customer_mas_industry;";
                        $stmt = $this->db->prepare($sql);
                        if ($stmt->execute()) {
                            $data = [
                                'status' => 'success',
