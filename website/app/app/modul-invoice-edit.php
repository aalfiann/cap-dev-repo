...
<head>
    ...
    <title><?php echo Core::lang('invoice_edit')?> - <?php echo Core::getInstance()->title?></title>
    <!--alerts CSS -->
...
<body class="fix-sidebar fix-header card-no-border">
            ...
            <div class="row page-titles">
                ...
                <div class="col-md-7 align-self-center">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"><?php echo Core::lang('invoice_edit')?></li>
                    </ol>
                </div>
                <div>
    ...
    <script>
        ...
        function selectedOption(){
            var selection = document.getElementById("selectoptstatus") !== null;
            if (selection){
                var selectBox = document.getElementById("selectoptstatus");
        ...
        function getDataInvoice(){
            $(function(){
                $.ajax({
                    url: Crypto.decode("<?php echo base64_encode(Core::getInstance()->api.'/invoice/data/read/'.$codeid.'/'.$datalogin['username'].'/'.$datalogin['token'])?>")+"?_="+randomText(5),
                    dataType: "json",
                    type: "GET",
                    success: function(data) {
                        if (data.status == "success"){
                            $('#from_id').val(data.result[0].Custom_id.BranchID.toUpperCase());
                            $('#from_name').val(data.result[0].From_name);
                            $('#from_name_company').val(data.result[0].From_name_company);
                            $('#from_address').val(data.result[0].From_address);
                            $('#from_phone').val(data.result[0].From_phone);
                            $('#from_fax').val(data.result[0].From_fax);
                            $('#from_email').val(data.result[0].From_email);
                            $('#from_website').val(data.result[0].From_website);

                            $('#to_id').val(data.result[0].Custom_id.CustomerID.toUpperCase());
                            $('#to_name').val(data.result[0].To_name);
                            $('#to_name_company').val(data.result[0].To_name_company);
                            $('#to_address').val(data.result[0].To_address);
                            $('#to_phone').val(data.result[0].To_phone);
                            $('#to_fax').val(data.result[0].To_fax);
                            $('#to_email').val(data.result[0].To_email);
                            $('#to_website').val(data.result[0].To_website);

                            $('#invoice_bank_name').val(data.result[0].Custom_field.Bank_name);
                            $('#invoice_bank_address').val(data.result[0].Custom_field.Bank_address);
                            $('#invoice_bank_account_name').val(data.result[0].Custom_field.Bank_account_name);
                            $('#invoice_bank_account_no').val(data.result[0].Custom_field.Bank_account_no);

                            $('#invoice_tax').val(data.result[0].Custom_field.Tax_rate);
                            $('#invoice_term').val(data.result[0].Term);
                            $('#invoice_signature').val(data.result[0].Signature);

                            $('#invoice_sub_total').val(data.result[0].Total_sub);
                            $('#invoice_tax_total').val(data.result[0].Custom_field.Tax_total);
                            $('#invoice_total').val(data.result[0].Total);

                            $.each(data.result[0].Data_table, function(index, value){
                                $("#tableinvoice").find('tbody').append(
                                    $('<tr id="tablerow'+index+'">').append(
                                        $('<td>').append('<a href="#" onclick="removeRow(\''+index+'\');return false;"><i class="mdi mdi-close"></i></a>'),
                                        $('<td>').append(value.date),
                                        $('<td>').append(value.po),
                                        $('<td>').append(value.desc),
                                        $('<td>').append(value.qty),
                                        $('<td>').append(value.amount),
                                        $('<td>').append(value.total)
                                    )
                                );
                            });
                            convertJSON();
                            setTimeout(function() {
                                $("#selectoptstatus").val(data.result[0].StatusID);
                            }, 2000);
                        } else {
                            swal({
        ...
        function getStatusOption(){
            $(function(){
                $.ajax({
				    url: Crypto.decode("<?php echo base64_encode(Core::getInstance()->api.'/invoice/option/data/status/'.$datalogin['token'])?>")+"?_="+randomText(60),
	    	    	dataType: 'json',
	    	    	type: 'GET',
		    		ifModified: true,
    			    	...
    			    	if (status === "success") {
					    	if (data.status == "success"){
                                $.each(data.results, function(i, item) {
                                    $("#selectoptstatus").append("<option value=\""+data.results[i].StatusID+"\">"+data.results[i].Status+"</option>");
                                });
    				    	}
    	    			}
	    		    },
        ...
        $(function(){
            ...
            function submitInvoice(e){
                ...
                disableClickButton(btn);
                $.ajax({
                    url: Crypto.decode("<?php echo base64_encode(Core::getInstance()->api.'/invoice/data/update')?>"),
                    data : {
                        Username: "<?php echo $datalogin['username']?>",
                        Token: "<?php echo $datalogin['token']?>",
                    ...
                    success: function(data) {
                        div.innerHTML = "";
                        if (data.status == "success"){
                            div.innerHTML = messageHtml("success","<?php echo Core::lang('core_process_update').' '.Core::lang('invoice').' '.Core::lang('status_success')?>");
                            swal({
                                title: "<?php echo Core::lang('update').' '.Core::lang('invoice').' '.Core::lang('status_success')?>",
                                text: data.message,
                                type: "success"
                            });
                            console.log("<?php echo Core::lang('core_process_update').' '.Core::lang('invoice').' '.Core::lang('status_success')?>");
                            that.on("submit", submitInvoice); /* add handler back after ajax */
                        } else {
                            div.innerHTML = messageHtml("danger","<?php echo Core::lang('core_process_update').' '.Core::lang('invoice').' '.Core::lang('status_failed')?>",data.message);
                            swal({
                                title: "<?php echo Core::lang('update').' '.Core::lang('invoice').' '.Core::lang('status_failed')?>",
                                text: data.message,
                                type: "error"
                            });
                            that.on("submit", submitInvoice); /* add handler back after ajax */
                    ...
                    error: function(x, e) {
                        var obj = JSON.parse(x.responseText);
                        div.innerHTML = messageHtml("danger","<?php echo Core::lang('core_process_update').' '.Core::lang('invoice').' '.Core::lang('status_failed')?>",obj.message);
                        swal({
                            title: "<?php echo Core::lang('update').' '.Core::lang('invoice').' '.Core::lang('status_failed')?>",
                            text: obj.message,
                            type: "error"
