...
<body class="fix-sidebar fix-header card-no-border">
    ...
    <script>
        ...
        function sendPodSuccess(){
            $(function(){
                $.ajax({
                    url: Crypto.decode("<?php echo base64_encode(Core::getInstance()->api.'/cargoagent/transaction/data/pod/delivered')?>"),
                    data : {
        ...
        function sendPodFailed(){
            $(function(){
                $.ajax({
                    url: Crypto.decode("<?php echo base64_encode(Core::getInstance()->api.'/cargoagent/transaction/data/pod/failed')?>"),
                    data : {
                        Username: "<?php echo $datalogin['username']?>",
                        Token: "<?php echo $datalogin['token']?>",
                        Waybill: $("#codeid").val(),
        ...
        function sendPodReturn(opt='1'){
            $(function(){
                ...
                switch(opt){
                    case '1':
                        urldata = Crypto.decode("<?php echo base64_encode(Core::getInstance()->api.'/cargoagent/transaction/data/pod/returned/consignor')?>");
                        break;
                    case '2':
                        urldata = Crypto.decode("<?php echo base64_encode(Core::getInstance()->api.'/cargoagent/transaction/data/pod/returned/consignee')?>");
                        break;
                    default:
                        urldata = Crypto.decode("<?php echo base64_encode(Core::getInstance()->api.'/cargoagent/transaction/data/pod/returned')?>");
                }
                $.ajax({
                    url: urldata,
                    data : {
