...
<head>
    ...
    <title><?php echo Core::lang('chart').' '.Core::lang('agent').' '.Core::lang('transaction')?> - <?php echo Core::getInstance()->title?></title>
</head>
        ...
        <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-themecolor"><?php echo Core::lang('chart').' '.Core::lang('agent').' '.Core::lang('transaction')?></h3>
                </div>
    ...
    <script>
        $.when(
            $.ajax({ /* Get file statistic start */
				type: "GET",
				url: Crypto.decode("<?php echo base64_encode(Core::getInstance()->api.'/cargoagent/transaction/stats/data/summary/'.date('Y').'/'.$datalogin['username'].'/'.$datalogin['token'])?>")+"?_="+randomText(60),
				dataType: "json",
        ...
        $.when(
            $.ajax({ /* Get gross profit chart start */
				type: "GET",
				url: Crypto.decode("<?php echo base64_encode(Core::getInstance()->api.'/cargoagent/transaction/stats/data/chart/'.date('Y').'/'.$datalogin['username'].'/'.$datalogin['token'])?>")+"?_="+randomText(60),
                dataType: "json"
			})
